...
It is important to be very thoughtful and precise in defining and developing a measure of the outcome to be predicted. In our motivating example, we want to predict employment, but we need to precisely define a measure of employment. When defining an outcome, the following considerations are essential:

1.  *What is important to the service provider's goals?* In our example, what is most important to the TANF program's goals for its clients?
...
The selected precise outcome measure is: [whether a client holds a job covered by unemployment insurance (UI) for the first three quarters after exiting the department's education and training program for TANF participants]{style="color:blue"}. The measure is coded as 1 when achieved and 0 otherwise.
...
3.  *Our employment information hinges on UI data.* These data encompass most jobs but possibly missing certain forms of work like gig employment and self-employment.
