---
title: "Learner training with code templates"
html:
  css: styles.css
---
...
All learners are trained in the following code chunk in `02_Learner_Training_and_Validation.Rmd`. The user does not need to enter or edit any code within the chunk. 
...
As commented in the above chunk, the code returns a couple of performance metrics (AUC-PR and AUC-ROC). We will return to explanations of these in a bit. First, let's focus on the other information that is returned, the predicted probabilities . For each observation (unit or row) in our data and for each learner, the code returns a predicted probability of the outcome. For example, imagine we are predicting whether a student will drop out of high school. Then, for a single student, we get the following information. 
```{r echo=FALSE}
