...
Gradient boosting algorithms, like Random Forest, are built on decision trees. However, gradient boosting takes a different approach for constructing trees.
...
<!-- -   **Flexibility:** It can produce a more interpretable model than Random Forest.  -->
...
-   **Memory consumption:** XGBoost can be more memory-intensive, especially when handling large datasets. 

-   **Handling categorical features:** While Random Forest can directly handle categorical variables, XGBoost requires them to be transformed into numerical format - creating multiple dummies, as discussed in "Data for PA: Part 2," (which is sometimes called "one-hot coding").
