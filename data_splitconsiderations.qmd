...
Ultimately, if the learner/model performs well in validation and generalizes well to the test data, the dataset is likely large enough. If the model overfits to the training data - it does not generalize well to the test data, one reason may be that the modeling approach was to complex for the size of the available data.
