...
2.  **To prioritize explainable models,** Lasso may be a good choice.
    ...
    SVM is quite opaque, and tabular foundation models are the least transparent of all the methods we have learned - with them, you can say very little about why any individual received the score they did.
...
6.  **If your dataset is small, consider a [tabular foundation model](methods_tfm.qmd).** This is the newest approach in this section and it works differently from the others: the model arrives already trained on millions of artificial datasets, so it needs less of your data to do well.
    Small datasets are the setting where the other ML algorithms most often fail to improve on a simple benchmark, so this is worth trying.
    Be aware of the trade-off, though - these models are the hardest of all to explain, so if transparency matters in your context, this may not be a viable choice no matter how it performs.

7.  **"Ensemble" approaches can harness multiple ML algorithms together.** While beyond the scope of this class and not implemented in the templates, data scientists sometimes harness the strengths of multiple ML algorithms by combining them in various ways.
...
8.  **Without ensembling, try a mix of ML strategies and predictor sets.** As was discussed in the [section](poc_compare.qmd) on setting up a predictive analytics proof-of-concept (which would be good to now review), I recommend specifying learners that incrementally add complexity.
