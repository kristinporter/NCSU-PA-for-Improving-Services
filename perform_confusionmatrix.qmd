...
If we specify a threshold, then we can convert each predicted probability into a predicted classification. We can then compare that predicted classification to the actual, observed classification (e.g. whether or not a student actually dropped out.) We can summarize the comparisons in 2x2 matrix called a **confusion matrix:**
...
Finally, we can also summarize these categories into many different rates:

*Rates based on total true, observed number (OP or ON):*
...
*Rates based on total predicted number (PP or PN):*
